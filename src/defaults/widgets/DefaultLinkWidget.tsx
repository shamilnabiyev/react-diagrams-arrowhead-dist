...
export class DefaultLinkWidget extends BaseWidget<DefaultLinkProps, DefaultLinkState> {
	...
	generateMarker(id: string) {
		return (
			<defs>
				<marker id={id} markerWidth="8" markerHeight="8" refX="3.5" refY="3" orient="auto"
						markerUnits="strokeWidth">
					<path d="M0,0 L0,6 L3.5,3 z"/>
				</marker>
			</defs>
		);
	}
	...
	generateLink(path: string, extraProps: any, id: string | number, extraParams?: any): JSX.Element {
		...
		if (extraParams && extraParams.startMarkerOnly === true && startMarker === true) {
			startMarkerId = Toolkit.UID();
			StartMarkerElement = this.tmpGenerateStartMarker(startMarkerId);
		}
		if (extraParams && extraParams.endMarkerOnly === true && endMarker === true) {
			endMarkerId = Toolkit.UID();
			EndMarkerElement = this.tmpGenerateEndMarker(endMarkerId);
		}
		//
	...
	render() {
		...
		if (paths.length === 0) {
			if (points.length === 2) {
				...
				paths.push(
					this.generateLink(
						Toolkit.generateLinePath(pointLeft, pointRight),
						{
							onMouseDown: event => {
								this.addPointToLink(event, 1);
							}
